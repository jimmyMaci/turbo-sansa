...
public class UsersBusinessService extends
		AbstractBusinessService<Users, Integer, UsersDao> implements
		UsersService {
	...
	public List<Roles> findRolesFromUser(final Users user) {
		final String hqlString = "select u.roles from Users u "
				+ "where u=:user";
		final Query query = getQuery(hqlString);
		query.setParameter("user", user);
	...
	public Users findUserWithEmail(final String email) {
		final String hqlString = "select u from Users u inner join u.userData.contactmethods cc "
				+ "where cc.contactmethod='EMAIL' "
				+ "and cc.contactmethod.contactvalue=:email";
		final Query query = getQuery(hqlString);
	...
	public Users findUserWithUsername(final String username) {
		final String hqlString = "select u from Users u "
	...
	@SuppressWarnings("unchecked")
	public List<Users> findUsers(Integer from, Gender searchGender,
			Integer until) {
		...
		final String hqlString = "select u from Users u "
				+ "where u.userData.gender=:gender "
//				+ "and u.userData.dateofbirth between :start and :end"
				+ "and u.userData.dateofbirth >= :start " 
				+ "and u.userData.dateofbirth <= :end"
				;
		final Query query = getQuery(hqlString);
	...
	@SuppressWarnings("unchecked")
	public List<Users> findUsers(Integer from, Gender searchGender,
			Integer until, final String geohash) {
		Date now = new Date(System.currentTimeMillis());
		Date start = CalculateDateUtils.substractYearsFromDate(now, until);
		Date end = CalculateDateUtils.substractYearsFromDate(now, from);
		
		final StringBuilder hqlString = new StringBuilder();
		hqlString.append("select u from Users u "
				+ "where u.userData.gender=:gender "
//				+ "and u.userData.dateofbirth between :start and :end"
				+ "and u.userData.dateofbirth >= :start " 
				+ "and u.userData.dateofbirth <= :end " 			
