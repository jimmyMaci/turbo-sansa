...
import address.book.init.DeZipcodeBean;
...
public class AddressesBusinessServiceTest extends AbstractTestNGSpringContextTests {
	...
	public void validateAddressesWithSameCityname() {
		...
		for (Iterator<Addresses> iterator = addresses.iterator(); iterator.hasNext();) {
			...
			List<Addresses> addressesSameCityName = addressesService.findAddressesWithSameCityname(address.getZipcode().getCountry(), address.getZipcode().getCity());
	...
	public void testGetInvalidAddresses(){
		...
		for (Addresses address : addresses) {
			System.out.println("to process:" + address.getZipcode().getZipcode() +" " +address.getZipcode().getCity());
			List<Addresses> addressesSameCityName = addressesService.findAddressesWithSameCityname(address.getZipcode().getCountry(), address.getZipcode().getCity());
//			List<Addresses> addressesSameCityName = addressesService.findAddressesWithSameZipcode(address.getZipcode().getCountry(), address.getZipcode().getZipcode());
			...
			for (Addresses address2 : addressesSameCityName) {
				if(address2.getGeohash().startsWith("u")){
					...
					System.out.println("processed:"+freshFromDb.getZipcode().getZipcode()+" "+freshFromDb.getZipcode().getCity());
					break;
				}
			}			
		}
//		for (Addresses address : addresses) {
//			Addresses freshFromDb = addressesService.get(address.getId());
//			GeoHashPoint ghp = new GeoHashPoint(freshFromDb.getLatitude(), freshFromDb.getLongitude());
...
//		}
	...
	public void valideAddressZipcodes() throws IOException{
		...
		for (DeZipcodeBean bean : deZipcodeBeanList) {
			List<Addresses> addresses = addressesService.find(country, bean.getZipcode(), bean.getCity());
			if(1<addresses.size()){
				System.out.println("ambiguous:"+bean);
				continue;
			}
			Addresses addressInDb = ListUtils.getFirst(addresses);
			...
			if(addressInDb != null) {
				...
				double distance = DistanceCalculator.distance(latDb, lngDb, latBean, lngBean, MeasuringUnit.KILOMETER);
				if(30.0 < distance) {
					GeoHashPoint point;
					try {
